...
class eltuxusb:
	...
	self.name_recording = ""
        ...
        events = { 'on_download_button_clicked': self.download,
                   ...
                   'on_checkbutton2_toggled': self.high_alarm,
                   'on_checkbutton3_toggled': self.low_alarm,
	...
	if self.dev1.init() == False:
		...
		if self.dev1.download() == False:
			...
			self.parse.data_translate(self.dev1.get_data(), self.dev1.get_config(), self.dev1.get_status(), self.result)
			self.widgets.get_widget('label2').set_text('Downloaded and stopped')


    def refresh(self, source=None, event=None):
	print "were in the refresh shit"
    ...
    def high_alarm(self, source=None, event=None):
    ...
    def low_alarm(self, source=None, event=None):	
	...
	if self.status == 0:
		...
		self.dev1.rd_buffer.cmd_typ = 0
		self.dev1.rd_buffer.flag_bit = [int(self.flag_bit_32, 2), int(self.flag_bit_33, 2)]
		self.dev1.rd_buffer.name = self.converted_name
		self.dev1.rd_buffer.sample_rate = self.converted_sample_rate
		self.dev1.rd_buffer.start_offset = self.offset_seconds_converted
		self.dev1.rd_buffer.unit = self.unit
		self.dev1.rd_buffer.hour = self.start_hour 
		self.dev1.rd_buffer.minute = self.start_min 
		self.dev1.rd_buffer.seconds = 0
		self.dev1.rd_buffer.day = self.start_day
		self.dev1.rd_buffer.month = self.start_month
		self.dev1.rd_buffer.year = self.start_year
		self.dev1.rd_buffer.hal = self.high_alarm_value_converted
		self.dev1.rd_buffer.lal = self.low_alarm_value_converted
		self.dev1.rd_buffer.sample_count = [0, 0]

		print self.unit

		self.buffer_new_record = self.dev1.rd_buffer.get_buffer()

		self.dev1.config_write(self.buffer_new_record)
